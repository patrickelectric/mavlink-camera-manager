...
use gst_rtsp_server::{prelude::*, RTSPSuspendMode, RTSPTransportMode};

use super::media_factory;
...
pub struct RTSPServer {
    ...
    port: u16,
    run: bool,
    pub path_to_factory: HashMap<String, media_factory::Factory>,
...
impl RTSPServer {
    ...
    pub fn add_pipeline(pipeline: &gst::Pipeline, path: &str) -> Result<()> {
        // Initialize the singleton before calling gst factory
        let mut rtsp_server = RTSP_SERVER.as_ref().lock().unwrap();

        let factory = media_factory::Factory::new(pipeline.upcast_ref());
        ...
        factory.set_suspend_mode(RTSPSuspendMode::Pause);
